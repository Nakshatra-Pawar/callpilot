 ...
 *
 * Chose these thresholds to bound runaway realtime-audio token spend
 * during exploratory sandbox sessions.
 *
    ...
    const tick = () => {
      ...
      if (!warned && elapsed >= warnMs) {
        ...
        toast.warning("Call ends in 1 minute", {
          description: "Auto-disconnect keeps sandbox sessions bounded.",
          duration: 8000,
        });
      }
      if (elapsed >= maxMs) {
