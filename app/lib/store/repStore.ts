 ...
 * Signed-in-rep store.
 *
 * CallPilot is built for one human at a time — the rep who's about to
...
const STORAGE_KEY = "callpilot:current-rep";
const LEGACY_STORAGE_KEY = "earshot:current-rep";
...
function hydrate(): Rep | null {
  ...
  try {
    ...
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(rep));
...
/**
 * First name only, for UI micro-copy ("Ready when you are, Jordan.").
