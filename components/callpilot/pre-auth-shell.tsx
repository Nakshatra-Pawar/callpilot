 ...
 * narrative one: before the rep tells CallPilot who they are, there is
 ...
 *     large italic "CallPilot" hero centered on the splash, but with
...
export function PreAuthShell({ children }: { children?: React.ReactNode }) {
  return (
    ...
    >
      ...
      <ThemeToggle className="absolute right-4 top-4 z-20 sm:right-6 lg:right-8" />

      {/* Footer signature — quiet product context for the pre-auth
          screen. Kept at a lower opacity so it doesn't pull attention
          from the modal. */}
