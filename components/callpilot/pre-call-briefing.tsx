 ...
 * for customers with zero CallPilot-recorded history: all core content
 ...
 *      touches from customer.recentActivity. If a CallPilot call with
...
type Props = {
  ...
  /**
   * When provided, the CallPilot call row acts as a button that restores
...
export function PreCallBriefing({
  ...
  if (!customer) {
    return (
      <aside
        ...
        className="callpilot-stagger-ledger flex flex-col gap-3"
      >
  ...
  return (
    <aside
      ...
      className={cn(
        "callpilot-stagger-ledger relative flex flex-col gap-4",
    ...
    >
      ...
      {/* Recent activity timeline. The last CallPilot call (if any) is
            ...
            (onOpenSummary ? (
              <li>
                <button
                  ...
                  aria-label={`Open full summary for last CallPilot call with ${customer.name}`}
                >
